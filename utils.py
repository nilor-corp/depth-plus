...
def get_bitsize_from_torch_type(torch_type):
    ...
    if (
        ...
        or torch_type == torch.float8_e5m2
        or torch_type == torch.float8_e4m3fnuz
    ):
...
def construct_output_paths(
    ...
    png_output_path = (
        f"{png_output_path}_8bit" if is_png_8bit else f"{png_output_path}_16bit"
    )
    ...
    exr_output_path = (
        f"{exr_output_path}_32bit" if is_exr_32bit else f"{exr_output_path}_16bit"
    )
...
def make_exr(filename_prefix, data=None):
    ...
    if num_channels == 1:
        ...
        default_names = ["R", "G", "B", "A"] + [
            f"Channel{i}" for i in range(4, num_channels)
        ]
